...
class CardsPlayScreen extends StatelessWidget {
  final int id;
  final int gameId;
  final String gameName;
  final bool toShowSession;
   CardsPlayScreen({super.key, required this.id, required this.gameId, required this.gameName, required this.toShowSession});
  ...
  Widget build(BuildContext context) {
    final CardsPlayController controller = Get.put(CardsPlayController());
                            ...
                            children: [
                                  ...
                                  SizedBox(
                                    ...
                                    child:  DropDownSearchField(
                                      ...
                                      suggestionsCallback: (pattern) async {
                                        switch (gameName) {
                                          case "Single Patti":
                                            return getSinglePatti.where((digit) => digit.contains(pattern)).toList();
                                          case "Double Patti":
                                            return getDoublePatti.where((digit) => digit.contains(pattern)).toList();
                                          case "Triple Patti":
                                            return getTriplePatti.where((digit) => digit.contains(pattern)).toList();
                                          default:
                                            return [];
                                        }
                                      },
                                      itemBuilder: (context, suggestion) {
                                        return ListTile(
                                          title: Text(suggestion,style: TextStyle(fontStyle: FontStyle.italic
                                          ),),
                                        );
                                      },
                                      onSuggestionSelected: (suggestion) {
                                        print(suggestion);
                                        controller.pattiController.text  = suggestion;
                                      },
                              ...
                              CustomLoginButton(
                                ...
                                onpressed: () {
                                  ...
                                  if(honecontroller.walletBalance.value.toInt()  <= 9  ){
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar( backgroundColor: Colors.red, content: Text('Insuffiecent Balance! Please Add Points ',selectionColor: Colors.green, )),
                                    );
                                  }else{
                                  if (_formKey.currentState?.validate() ?? false) {
                                    if(gameName == "Single Patti"){
                                      if( getSinglePatti.contains( controller.pattiController.text)){
                                        print("object");

                                        controller.listToSendDB.add(SendDigitsBidModel(
                                        ...
                                        showinvalidDigitsSnackbar( context,"Your bid has been added to list successfully" ,false )     ;
                                      }else{
                                        print("noo");
                                        showinvalidDigitsSnackbar( context,"Please select or enter the patti numbers to add bid" ,true)     ;
                                        print(controller.pattiController.text);
                                      }
                                    }
                                    else  if ( gameName == "Double Patti"){
                                      if( getDoublePatti.contains( controller.pattiController.text)){
                                        print("object");
                                        print(controller.pattiController.text);
                                        controller.list.add(Toshowusermodel(
                                          controller.sessionStatus.value,
                                        ...
                                        print("noo");
                                        showinvalidDigitsSnackbar( context,"You have selected invalid patti Number" ,true)     ;
                                        print(controller.pattiController.text);
                                      }
                                    }
                                    else  if ( gameName == "Triple Patti"){
                                      if( getTriplePatti.contains( controller.pattiController.text)){
                                        print("object");
                                        print(controller.pattiController.text);
                                        controller.list.add(Toshowusermodel(
                                          controller.sessionStatus.value,
                      ...
                      child: CustomSubmitButton(
                        ...
                        onpressed: () {
                          if( controller.listToSendDB.length >= 1  ){
                            ...
                            print(controller.listToSendDB.length);
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar( backgroundColor: Colors.red, content: Text('Please Add Your Bid to Submit',selectionColor: Colors.green, )),
                            );
                          }
                        },
                      ),
